...
In this skill, you will attempt to categorize transactions of a dataset using the existing reference rules. If the categorization fails for a transaction, you will identify a closely matching rule in the rule files of `data/reference` (`rules.json` and `rules.<topic>.json`), create an overlay variant of it that matches the uncategorized transaction and add it to the dataset-specific rules.
...
5. **Adapt the rule**: Extend the matching reference rule directly in the `data/reference` file that holds it so it also covers the uncategorized transaction (e.g. add the merchant name to the `merchants` list).

6. **Override the rule**: Add a rule to the fitting rule file of `data/[dataset]` (the `rules.<topic>.json` for its topic, or `rules.json` if none fits) that matches the uncategorized transaction. A key must be unique across all of the dataset's rule files. Rules are identified by `key`, not `id`:
